# Phase 21 — Deploy Infrastructure (Shared CAX11) (SPEC seed)

**Status:** SEED — produced by the v1.2 → v1.3 pivot on 2026-05-06. Renumbered from Phase 20 → 21 on 2026-05-11 when Phase 18 (Region Variants) was inserted. Run `/gsd-spec-phase 21` to expand.
...
- **New domain** — TLD chosen in Phase 22 (e.g. `lol-companion.gg` placeholder); flag the `[[values-charter#2]]` "EU tools preferred" value if non-EU TLD is chosen.
...
4. **DNS** — once domain is registered (Phase 22), point A/AAAA records at the CAX11's IP. Wait for Caddy to provision Let's Encrypt cert.
...
7. RUNBOOK.md covers: first deploy, regular deploy, rollback, secret rotation, log access (`journalctl -u lol_team_companion`), backup-restore (handed off to Phase 20 script), DNS change, cert troubleshooting.
...
TBD — produced by `/gsd-plan-phase 21`. Likely 2-3 plans:
- 21-01: infra/ files + setup-host.sh + first cold deploy
- 21-02: just deploy recipe + post-deploy smoke
- 21-03: RUNBOOK + rollback + secret rotation

## Depends on

- Phase 20 (Production Hardening) — env-driven config required
- Phase 22 (Compliance & Transparency) — domain registration; can proceed in parallel until DNS step
